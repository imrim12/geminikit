...
import { PuppeteerLifeCycleEvent, ScreenshotOptions } from 'puppeteer';
...
async function screenshot() {
  ...
  try {
    ...
    if (args.quality) {
      screenshotOptions.quality = parseInt(args.quality as string);
    }
    ...
    if (args['no-compress'] !== 'true') {
      const maxSize = args['max-size'] ? parseFloat(args['max-size'] as string) : 5;
