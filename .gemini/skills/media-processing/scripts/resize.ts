...
async function gatherImages(inputs: string[]): Promise<string[]> {
  const images: string[] = []
...
function buildResizeCommand(
  ...
  if (strategy === 'fit') {
    ...
    if (!width || !height)
      throw new Error('Both width and height required for \'fill\'')
    cmd.push(
      '-resize',
      `${width}x${height}^`,
      '-gravity',
    ...
    if (!width || !height)
      throw new Error('Both width and height required for \'cover\'')
    cmd.push('-resize', `${width}x${height}^`)
  }
  else if (strategy === 'exact') {
    if (!width || !height)
      throw new Error('Both width and height required for \'exact\'')
    cmd.push('-resize', `${width}x${height}!`)
  }
...
yargs(hideBin(process.argv))
  .command(
    ...
    async (argv) => {
      ...
      const images = await gatherImages(argv.inputs as string[])
      if (images.length === 0) {
