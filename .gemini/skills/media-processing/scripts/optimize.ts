...
async function getVideoInfo(filePath: string): Promise<VideoInfo | null> {
  try {
    ...
    const fps = fpsParts.length === 2 ? Number.parseInt(fpsParts[0]) / Number.parseInt(fpsParts[1]) : 0
    ...
    return {
      ...
      audioBitrate: Number.parseInt(audioStream?.bit_rate || '0'),
    }
  }
  catch {
    return null
...
async function optimizeVideo(
  ...
  if (options.twoPass) {
    ...
    // Pass 1
    const pass1Cmd = [...cmd, '-c:v', 'libx264', '-preset', options.preset, '-b:v', targetBitrate.toString(), '-pass', '1', '-an', '-f', 'null', process.platform === 'win32' ? 'NUL' : '/dev/null',
    ]
